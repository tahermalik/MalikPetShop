...
export async function getProductHTMLFile(req, res) {
    try {
        const productId = req?.params?.id;
        const productData = await Product.findById(productId)
            .select("cleanProductName originalPrice discountValue discountType image description brand category pet")
            .lean();
        ...
        // Use first variation's data (index 0)
        const price = productData.originalPrice?.[0];
        const discount = productData.discountValue?.[0];
        const discountType = productData.discountType?.[0];
        ...
        const structuredData = {
            ...
            "@type": "Product",
            "name": productData.cleanProductName,
            "image": [image],
            "description": productData.description || productData.cleanProductName,
            "brand": {
                "@type": "Brand",
                "name": productData.brand || "Your Brand"
            },
            "offers": {
                "@type": "Offer",
                "priceCurrency": "INR",
        ...
        const html = `
            ...
            <head>
                ...
                <title>${productData.cleanProductName}</title>
                <meta name="description" content="${productData.description || productData.cleanProductName}" />
                <meta property="og:title" content="${productData.cleanProductName}" />
            ...
            <body>
                ...
                <img src="${image}" alt="${productData.cleanProductName}" />
            </body>
            </html>
        `;
        ...
        return res.status(500).json({ message: "Server side error while generating html file" });
    }
}
